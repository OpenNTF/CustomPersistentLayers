...
public class ModelBase extends ModelBaseAnnotation {
	 ...
	 * "entity must be new");
	 */
	private static final long serialVersionUID = -126355568954326934L;
	/**
	 * 
	 */
	...
	protected DominoDocument initDoc(Object docObj) {
		...
		try {
			if (!(docObj instanceof DominoDocument)) {
				...
				doc.replaceItemValue(ITEM_FORM_NAME, formName);
				// convert a document to dominoDocument
				// use another constructor instead
				dominoDoc = DominoDocument.wrap("", doc, "both", "force", true,
						"", "");
			} else
	...
	protected Database getDatabase() {
		return DominoUtils.getCurrentDatabase();
	}
